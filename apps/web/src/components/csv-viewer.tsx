...
import { db, Job, jobs } from 'database';
import { createSignal, For, Show, createMemo } from 'solid-js';

// const getJobs = query(async () => {
//   const response = await fetch('http://localhost:3000/api/jobs');
//   // const response = await fetch('/api/jobs');
//   // fetch(`/api/jobs/${id}?summary=${options.summary || false}`).then((r) =>
...
interface TableData {
  headers: string[];
  rows: Record<string, Job[keyof Job]>[];
}

const CSVViewer = () => {
  const jobs = createAsync(() => getJobs());

  const [tableData] = createSignal<TableData>({
    headers: Object.keys(jobs()?.[0] || {}),
    rows: jobs() || [],
  });
